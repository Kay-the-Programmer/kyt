...
const PortfolioScroll = React.forwardRef<HTMLDivElement>((props, ref) => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // 2. Mobile Reveal Logic (Refined with Batch)
      mm.add("(max-width: 1023px)", () => {
        // Handle standard reveal targets (cards, text blocks)
        ...
        // Handle splitting text letter reveals separately for finer control
        const letters = gsap.utils.toArray<HTMLElement>('.letter-reveal');
        if (letters.length) {
          gsap.set(letters, { opacity: 0, y: 30, scale: 0.9, filter: 'blur(4px)' });
          ScrollTrigger.batch(letters, {
            start: 'top 85%',
            onEnter: (batch) => gsap.to(batch, {
              opacity: 1, y: 0, scale: 1, filter: 'blur(0px)',
              stagger: 0.02, duration: 0.6, ease: 'back.out(1.2)', overwrite: true
            })
          });
        }
      });
    ...
    <div ref={innerRef} className="portfolio-scroll-wrapper relative">
      ...
      <div className="fixed right-8 md:right-12 top-1/2 -translate-y-1/2 z-[70] hidden lg:flex flex-col space-y-8">
        {[0, 1, 2, 3].map((idx) => (
          ...
          >
            <span className={`absolute right-8 text-[9px] font-black uppercase tracking-[0.4em] transition-all duration-500 whitespace-nowrap pointer-events-none ${activePanel === idx ? 'opacity-100 translate-x-0 text-blue-600' : 'opacity-0 translate-x-4 text-gray-400'
              }`}>
              {['SalePilot Hub', 'System Mastery', 'Vision Logic', 'Absolute Impact'][idx]}
            </span>
            <div className={`w-2.5 h-2.5 rounded-full transition-all duration-700 border ${activePanel === idx
